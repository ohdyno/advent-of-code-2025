...
(defn extract-numbers [max-lengths input-line]
  (loop [line input-line
         [length & remaining-length] max-lengths
         numbers []]
    (if (nil? length) numbers
                      (recur (drop (inc length) line) remaining-length (conj numbers (take length line)))))
  )

(defn find-max-length-per-column [values]
...
(with-open [rdr (io/reader (io/resource "day6/input.txt"))]
  (let [input-lines (line-seq rdr)]
    (comment (assert (test/is (= 707 (time (process input-lines)))))
             (assert (test/is (= 361615643045059N nil))))
    (second input-lines)
    ))